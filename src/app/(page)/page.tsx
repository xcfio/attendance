...
import { useExtraStudents } from "./use-extra-students"
import { useAttendanceExport } from "./use-attendance-export"
...
function getSubjectData(): Array<[number, string]> {
    const subjects: Array<[number, string]> = []
    for (const [semester, department] of Subject) {
        if (semester !== Semester) continue
        for (const [code, name] of department) subjects.push([code, name])
    }
    return subjects
}
...
export default function StudentPage() {
    ...
    const subjects = useMemo(() => getSubjectData(), [])

    const [date, setDate] = useState<Temporal.PlainDate | undefined>(Temporal.Now.plainDateISO())
    const [selectedSubject, setSelectedSubject] = useState("")
    const [rowSelection, setRowSelection] = useState<RowSelectionState>({})

    const extra = useExtraStudents(rowSelection, setRowSelection)
    const { copy, save } = useAttendanceExport()

    function exportArgs() {
        return {
            subject: selectedSubject,
            time: date?.toLocaleString("en-UK", { day: "2-digit", month: "2-digit", year: "numeric" }),
            selectedRows: data.filter((row) => rowSelection[row.roll]),
            extraRolls: extra.selectedRolls()
        }
    }
        ...
        <div className="relative w-full flex flex-1 items-center justify-center overflow-hidden bg-linear-to-br from-background via-muted/40 to-background">
            ...
            <Card className="[--card-spacing:--spacing(4)]! sm:[--card-spacing:--spacing(6)]! m-2 md:m-4 lg:m-8 xl:m-16 max-w-4xl flex flex-1">
                            ...
                            <TableRow key={`extra-${index}`}>
                                ...
                                <TableCell className="font-mono">
                                    <Input
                                        placeholder="Roll"
                                        value={entry.roll}
                                        onChange={(e) => extra.update(index, "roll", e.target.value)}
                                        onBlur={() => extra.handleRollBlur(index)}
                                ...
                                <TableCell>
                                    <div className="flex items-center gap-2">
                                        <Input
                                            ...
                                            value={entry.name}
                                            onChange={(e) => extra.update(index, "name", e.target.value)}
                                            className="h-7 flex-1 px-2 text-sm"
                                        />
                                        {(entry.roll !== "" || extra.extraStudents.length > 1) && (
                                            <button
                                                type="button"
                ...
                <CardFooter className="flex flex-row justify-start items-start gap-2">
                    <Button variant="default" className="w-fit" onClick={() => save(exportArgs())}>
                        Save
                    </Button>
                    <Button variant="default" className="w-fit" onClick={() => void copy(exportArgs())}>
                        Copy
                    </Button>
                </CardFooter>
            </Card>
